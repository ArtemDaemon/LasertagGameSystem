...
<div class="b-gamesettings">
    ...
    <div>Режим точек: Уничтожение домов</div>
...
<div class="b-table__wrap">
    ...
    <table>
        ...
        <?php foreach($points as $point): ?>
            <?php $preset = false;?>
            <?php if(!$pointSetup = $mission->getPointSetup($point['id'])): ?>
                <?php $point['status'] = \entities\Point::STATUS_NOT_PLAY ?>
            <?php endif ?>
            <?php $preset = $game->getPresetMain($pointSetup['MainPresetId']); ?>
            <tr>
                <td><?= $point['id'] ?></td>
                <td><?= $point['Name'] ?></td>
                <td><?= \classes\PointHelper::drawTakeTager($point['takeTagerId']) ?></td>
                <td><?= \classes\PointHelper::statusName($point['status'], $mission->pointsType(), \entities\Point::TYPE_4) ?></td>
                <td><?= \classes\PointHelper::drawHealth($point['health'], ($preset['SumDamage'] ?: false)) ?></td>
            </tr>
        <?php endforeach ?>
        </tbody>
    </table>
